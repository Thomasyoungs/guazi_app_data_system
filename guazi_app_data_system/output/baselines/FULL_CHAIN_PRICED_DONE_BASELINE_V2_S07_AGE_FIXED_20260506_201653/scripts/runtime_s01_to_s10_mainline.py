...
def _s07_exact_age_text(snapshot: dict[str, Any], target_age: int | None) -> str | None:
    ...
    for text in [str(snapshot.get("visible_blob") or "")] + [str(label) for node in snapshot.get("nodes", []) for label in node.get("labels", [])]:
...
def _exact_age_confirmed(snapshot: dict[str, Any], target_age: int | None) -> bool:
    if target_age is None:
        return False
    if _s07_exact_age_text(snapshot, target_age):
        return True
    left_age, right_age = _snapshot_age_pair(snapshot)
    if left_age == target_age and right_age == target_age:
        return True
    slider_left, slider_right = _s07_age_range_from_slider_positions(snapshot)
    if slider_left == target_age and slider_right == target_age:
        return True
...
def _s07_age_handle_selection(snapshot: dict[str, Any]) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    for node in snapshot.get("nodes", []):
        ...
        elif track_node_bounds is None and not (tick_y - 80 <= cy <= tick_y + 140):
            reason = "outside_age_tick_y_range"
        elif not (45 <= width <= 180 and 80 <= height <= 240):
            reason = "not_slider_handle_size"
        if reason:
            if (
                min_x <= cx <= max_x
                and tick_y - 120 <= cy <= tick_y + 160
                and 45 <= width <= 220
                and 40 <= height <= 260
            ):
                rejected.append(
        ...
        candidates.append(node)
    if len(candidates) <= 2:
        return sorted(candidates, key=lambda item: _center(item["bounds"])[0]), candidates, rejected
    # Prefer the widest separated pair that overlaps the age track hotzone.
    candidates = sorted(candidates, key=lambda item: _center(item["bounds"])[0])
    return [candidates[0], candidates[-1]], candidates, rejected
...
def _s07_age_slider_points(snapshot: dict[str, Any], target_age: int | None) -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "right_drag_y_source": "track_center_y" if _s07_age_track_bounds(snapshot) else None,
    }
...
def _s07_age_slider_evidence(snapshot: dict[str, Any], target_age: int | None) -> dict[str, Any]:
    ...
    exact_confirmed = bool(exact_text) or slider_overlap_confirmed
    ...
    evidence: dict[str, Any] = {
        ...
        "age_confirm_source": "exact_age_text" if exact_text else ("slider_positions" if slider_overlap_confirmed else None),
        ...
        "right_drag_y_source": "track_center_y" if track_bounds else None,
        "right_handle_source": "age_panel_handle" if selected_handles else None,
    }
    if centers:
...
def _set_exact_age_from_ticks(context: dict[str, Any], snapshot: dict[str, Any], target_age: int | None) -> dict[str, Any]:
    ...
    evidence.update(
        {
            ...
            "age_interaction_strategy_priority": ["right_first", "long_press_drag", "segmented_drag", "track_based_drag"],
            "age_strategy_attempts": [],
            "right_slider_max_retry": 2,
            "right_slider_recalc_bounds_each_attempt": True,
            "right_slider_fresh_before_each_attempt": True,
            "age_strategy_dynamic_bounds": True,
    ...
    if target_point is None or not _s07_age_tick_nodes(current):
        evidence["failure_reason"] = "S07_AGE_SLIDER_TARGET_NOT_FOUND"
        return evidence
    ...
    def record_attempt(
        *,
        ...
        record = {
            ...
            "exact_confirmed_after_attempt": _exact_age_confirmed(after, target_age),
        ...
        if side == "right":
            ...
            evidence["right_drag_y_source"] = "track_center_y" if before_points.get("age_track_bounds") else None
    ...
    def drag_age_handle(
        ...
        *,
        ...
        ex, ey = target_point
        started = time.perf_counter()
        client.run(["shell", "input", "swipe", str(sx), str(sy), str(ex), str(ey), str(duration_ms)], timeout=20)
        move_ms = int((time.perf_counter() - started) * 1000)
        if side == "right":
            evidence["right_move_ms"] += move_ms
            evidence["right_slider_moved"] = True
        else:
            evidence["left_move_ms"] += move_ms
            evidence["left_slider_moved"] = True
        time.sleep(0.25)
        after = _capture(client, f"s07_age_{strategy}_{side}_{attempt_index}_{_timestamp()}")
    ...
    def mark_success_if_exact(current_snapshot: dict[str, Any]) -> bool:
        if not _exact_age_confirmed(current_snapshot, target_age):
            return False
        ...
        evidence["age_confirm_source_after_action"] = "exact_age_text" if _s07_exact_age_text(current_snapshot, target_age) else "slider_positions"
        return True
    ...
    if right_age_before is not None and target_age is not None and right_age_before > target_age:
    ...
    if right_age_current is not None and target_age is not None and right_age_current != target_age:
    ...
    if right_age_current is not None and target_age is not None and right_age_current > target_age:
        ...
        for attempt_index, age in enumerate(intermediate_ages[:3], start=1):
            ...
            if mark_success_if_exact(current):
                return evidence
        current = move_left_if_needed(current, strategy="segmented_drag", attempt_index=4, duration_ms=450)
        if mark_success_if_exact(current):
            return evidence
    ...
    if not evidence.get("success"):
        ...
        evidence["left_age_after"] = left_after
        evidence["right_age_after"] = right_after
        evidence["failure_reason"] = "S07_RIGHT_AGE_SLIDER_MOVE_NO_EFFECT" if right_after != target_age else "S07_AGE_EXACT_CONFIRM_FAILED"
    return evidence
...
def handle_s07(context: dict[str, Any], snapshot: dict[str, Any]) -> tuple[str, dict[str, Any]] | dict[str, Any]:
    ...
    _add_runtime_timing(
        ...
        extra={
            ...
            "s07_current_xml_reused_for_color_check": True,
            "s07_visible_text_digest_reused": True,
            "s07_xml_parse_count_for_color_check": 1,
    ...
    _add_runtime_timing(
        ...
        extra={
            ...
            "s07_color_node_reused_for_click": True,
    ...
    age_tab_ms = int((time.perf_counter() - t0) * 1000)
    ...
    _add_runtime_timing(
        ...
        page_name="S07",
        action_name="short_poll_confirm_exact_age",
        transition_wait_ms=int((time.perf_counter() - confirm_age_started) * 1000),
        snapshot=snapshot,
        extra={
            **age_action,
            "target_age": params.get("target_age_years"),
            "exact_age_confirmed": _exact_age_confirmed(snapshot, params.get("target_age_years")),
            ...
            "age_confirm_source": "exact_age_text" if _s07_exact_age_text(snapshot, params.get("target_age_years")) else ("slider_positions" if _s07_age_range_from_slider_positions(snapshot) == (params.get("target_age_years"), params.get("target_age_years")) else None),
            ...
            "age_xml_stale_after_move": not _exact_age_confirmed(snapshot, params.get("target_age_years")),
            "reason_category": "S07_AGE_EXACT_CONFIRM_SLOW",
            "reason_detail": "AGE_FILTER_DONE is gated by a fresh exact-age confirmation",
            "solution": "keep confirmation but stop polling as soon as target_age-target_age evidence appears",
        },
    )
    if not _exact_age_confirmed(snapshot, params.get("target_age_years")):
        issue = _record_issue(
            issues,
            "S07_AGE_EXACT_CONFIRM_FAILED",
            "S07",
