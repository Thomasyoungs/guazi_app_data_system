...
The official input source is Feishu. Local mock files are accepted only when a
caller explicitly marks the data as simulation-only.
"""
...
OPTIONAL_ACCIDENT_FIELDS = ("accident_count", "max_accident_amount")
TARGET_INPUT_FIELDS = (
    "task_id",
    "brand",
    "series",
...
class TargetCarTask:
    ...
    def to_target_car(self) -> TargetCar:
        if self.app_flow_blocked or self.pricing_blocked:
            raise TaskContractError("Cannot convert blocked task to TargetCar.")
        return TargetCar(
            ...
            registration_date=str(self.registration_date_raw),
            mileage_10k_km=float(self.mileage_10k_km),
            transfer_count=int(self.transfer_count),
            condition_text=str(self.condition_text),
            accident_count=self.accident_count,
            max_accident_amount=self.max_accident_amount,
        )


def derive_vehicle_year(registration_date: str | int | None) -> int:
    """Derive the matching year while preserving raw registration date elsewhere."""
...
def colors_match_exact(target_color: str | None, candidate_color: str | None) -> bool:
    """Strict color comparison: no family merging or synonym expansion."""
...
def normalize_target_task(
    ...
    *,
    ...
    if task_source == "manual":
        raise TaskContractError("task_id and target task data must not be manually entered.")
    if str(raw.get("task_id_source") or "") == "manual":
        raise TaskContractError("task_id must come from Feishu or explicit simulation fixture, not manual input.")
    ...
    if task_source in REAL_DEVICE_OPERATION_SOURCES and is_simulation_only:
        raise TaskContractError(f"{task_source} target tasks must not be marked simulation_only=true.")
    ...
    allow_real_device_operation = _allows_real_device_operation(task_source, is_simulation_only, bool(app_block_reasons))
    ...
    return TargetCarTask(
        ...
        allow_real_device_operation=allow_real_device_operation,
    )
