...
def _extract_all_history_repair_counts(texts: list[str]) -> dict[str, int | None]:
    return {region_name: _extract_adjacent_history_repair_count(texts, region_name) for region_name in S13_REGION_ORDER}
...
def handle_s13(context: dict[str, Any], snapshot: dict[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    for region_name in S13_REGION_ORDER:
        ...
        read_start = time.perf_counter()
        counts_summary = _extract_all_history_repair_counts(snapshot.get("visible_texts", []))
